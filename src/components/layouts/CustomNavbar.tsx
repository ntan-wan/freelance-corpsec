  ...
  return (
    <nav className={cn(style.base, sticky && style.sticky, className)}>
      ...
      <ul className="flex items-center gap-4">
        {menuItems.map((item) => (
          <li className="hidden lg:block" key={item.label}>{item.label}</li>
        ))}
        <li>
          <CustomButton label="Start Now" />
        </li>
		<li className="lg:hidden">
          <CustomButton icon="Bars3Icon" className="p-1" onClick={() => {setState({sidebarIsOpen : !globalState.sidebarIsOpen})}}/>
		</li>
      </ul>
    </nav>
  );
